...
uint8_t countVoltageGT360(int16_t vr, int16_t vy, int16_t vb);
uint8_t countVoltageGT24(int16_t vr, int16_t vy, int16_t vb);
...
#ifdef LAST_TAMPER_DETAILS_REQUIRED
// before recording the latest tamper, we should saved the data
...
// time and energy have been recorded 1 second after the event was detected
// the following function returns the date and time
		earlierTamperDetails.tamperTime = readInstantaneousDateTime(mytmprdata.latestTamperCode); 
...
void clear_tamperEvent_image(uint32_t flagmask)
{
  ...
  mytmprdata.tamperEvent_on &= ~flagmask;
	...
	{// only restored tamper event is to be recorded - not others such as transaction, power fail etc.
		EPR_Read(LAST_RESTORED_DETAILS_ADDRESS,(uint8_t *)&earlierTamperDetails,sizeof(earlierTamperDetails));
		EPR_Write(SECOND_LAST_RESTORED_DETAILS_ADDRESS,(uint8_t *)&earlierTamperDetails,sizeof(earlierTamperDetails));

		earlierTamperDetails.tamperCode = eventCodeBit;
//		earlierTamperDetails.tamperTime=rtc;
// time and energy have been recorded 1 second after the event was detected
// the following function returns the date and time
		earlierTamperDetails.tamperTime = readInstantaneousDateTime(eventCodeBit); 
		
		EPR_Write(LAST_RESTORED_DETAILS_ADDRESS,(uint8_t *)&earlierTamperDetails,sizeof(earlierTamperDetails));
	}
#endif		
...
// check for special case
			if(flagmask==MAGNET_MASK)
			{// we have a DC magnet waiting to be recorded
  			if((mytmprdata.tamperEvent_image&AC_MAGNET_MASK) == 0)
				{// Ac magnet tamper is not recorded
					*ptr=0;
					set_tamperEvent_image(flagmask);
				}
				else
				{// AC magnet tamper is already recorded - do nothing
    			*ptr = 0;
				}
			}
			else if(flagmask==AC_MAGNET_MASK)
			{// we have a AC magnet waiting to be recorded
  			if((mytmprdata.tamperEvent_image&MAGNET_MASK) == 0)
				{// DC magnet tamper is not recorded
					*ptr=0;
					set_tamperEvent_image(flagmask);
				}
				else
				{// DC magnet tamper is already recorded - do nothing
    			*ptr = 0;
				}
			}
			else
			{// it is neither AC magnet or DC magnet
	      *ptr = 0;
	      set_tamperEvent_image(flagmask);
			}
    }
...
uint8_t countVoltageGT360(int16_t vr, int16_t vy, int16_t vb)
{// returns count of number of phases GT 360
	uint8_t retval=0;	// false
...
void performTamperProcessing(void){
  ...
  int16_t v[EM_NUM_OF_PHASE];	// one decimal
	int16_t pf[EM_NUM_OF_PHASE];	// three decimals
  int16_t ivector;
...
//	float32_t tempFloat;
	...
	uint8_t	ct_open_ctr;
	uint8_t phasesGT300; // this variable holds count
	uint8_t phasesGT360; // this variable holds count
	uint8_t phasesLT50;	// this variable returns the count of number of phases < 50 V 
	uint8_t phasesGT180LT276;
	uint8_t phasesGT276LT360;
//	uint8_t currentsGT95;
	uint8_t currentsGT45;
	uint8_t currentsGT145;
	uint8_t phasesGT144LT276;
	uint8_t phasesGT24;	// no of phases with more than 24 Volts
...
//-----------------------------------------------------------------------------------------	
  ...
  i[LINE_PHASE_B] = (int16_t)(EM_EXT_GetIRMS(LINE_PHASE_B)*100);
	...
	pf[LINE_PHASE_R] = (int16_t)(EM_GetPowerFactor(LINE_PHASE_R)*1000);
	if(pf[LINE_PHASE_R]<0)
		pf[LINE_PHASE_R]*=-1;
	
	pf[LINE_PHASE_Y] = (int16_t)(EM_GetPowerFactor(LINE_PHASE_Y)*1000);
	if(pf[LINE_PHASE_Y]<0)
		pf[LINE_PHASE_Y]*=-1;
	
	pf[LINE_PHASE_B] = (int16_t)(EM_GetPowerFactor(LINE_PHASE_B)*1000);
	if(pf[LINE_PHASE_B]<0)
		pf[LINE_PHASE_B]*=-1;
...
//	minValue = findMin(v[LINE_PHASE_R],v[LINE_PHASE_Y],v[LINE_PHASE_B]);
	...
	phasesGT360 = countVoltageGT360(v[LINE_PHASE_R],v[LINE_PHASE_Y],v[LINE_PHASE_B]);	// added for PVVNL
	...
	phasesGT276LT360 = countVoltageGT276LT300(v[LINE_PHASE_R],v[LINE_PHASE_Y],v[LINE_PHASE_B]);
//	currentsGT95 = countCurrentGT95(i[LINE_PHASE_R],i[LINE_PHASE_Y],i[LINE_PHASE_B]);
	currentsGT45 = countCurrentGT45(i[LINE_PHASE_R],i[LINE_PHASE_Y],i[LINE_PHASE_B]);
	currentsGT145 = countCurrentGT145(i[LINE_PHASE_R],i[LINE_PHASE_Y],i[LINE_PHASE_B]);
	phasesGT144LT276 = countVoltageGT144LT276(v[LINE_PHASE_R],v[LINE_PHASE_Y],v[LINE_PHASE_B]);
	phasesGT24 = countVoltageGT24(v[LINE_PHASE_R],v[LINE_PHASE_Y],v[LINE_PHASE_B]);	// returns the count of phases > 24
...
//	if(phasesGT300<2)
	{/* no two voltages are greater than 350 - one might be but not two of them*/
		...
		{
		if (v[LINE_PHASE_R] < 1315)
		{
	    check_setTamper(&missingPotentialCtr[LINE_PHASE_R], computeMask(MISSING_POTENTIAL_R),TAMPER_PERSISTENCE_ON, 0, RECORD_DELAY);
			misspotoccurencectr++;
		}
		
		if (v[LINE_PHASE_R] > 1325)
		{
	    check_clearTamper(&missingPotentialCtr[LINE_PHASE_R], computeMask(MISSING_POTENTIAL_R),TAMPER_PERSISTENCE_OFF, 0, RECORD_DELAY);
		}

		if (v[LINE_PHASE_Y] < 1315)
		{
	    check_setTamper(&missingPotentialCtr[LINE_PHASE_Y], computeMask(MISSING_POTENTIAL_Y),TAMPER_PERSISTENCE_ON, 1, RECORD_DELAY);
			misspotoccurencectr++;
		}
		
		if (v[LINE_PHASE_Y] > 1325)
		{
	    check_clearTamper(&missingPotentialCtr[LINE_PHASE_Y], computeMask(MISSING_POTENTIAL_Y),TAMPER_PERSISTENCE_OFF, 1, RECORD_DELAY);
		}
		
		if (v[LINE_PHASE_B] < 1315)
		{
	    check_setTamper(&missingPotentialCtr[LINE_PHASE_B], computeMask(MISSING_POTENTIAL_B),TAMPER_PERSISTENCE_ON,2,RECORD_DELAY);
			misspotoccurencectr++;
		}
		
		if (v[LINE_PHASE_B] > 1325)
		{
...
// to determine chopped ac injection in neutral call the following function	
	...
	if((phasesGT300==2)&&(currentsGT145==3))
...
if(((g_TamperStatus&HALLSENS_MASK)==0)&&((g_TamperStatus&ACMAG_MASK)==0))
{
	if((phasesGT360>=1)||(freqDisturbed==1))	// PVVNL wants 360 volts
	{
    check_setTamper(&neutralDisturbanceCtr, computeMask(NEUTRAL_DISTURBANCE),20,9,RECORD_DELAY);
#ifdef ND_POWER_ON_ACTUALS		
	...
	{// here phasesGT300 can be 0,1, or 3(Really?)
		...
		{
    	check_clearTamper(&neutralDisturbanceCtr, computeMask(NEUTRAL_DISTURBANCE),20,9, RECORD_DELAY);
...
//				else
//					twoWireOperation=0;
// two wire operation is being checked even when there is MISSPOT - hence MISSPOT will get logged
			}
		}
	}
}
...
// Over Voltage & Low voltage & Voltage Unbalance and REVERSE Current

//	if(0==(g_TamperStatus&MISSPOT_MASK)
// 55%Vref = 132V,  75%Vref = 180V
// MVVNL defines Low Voltage occurence as :
// (Vph > 55%Vref && Vph < 75%Vref) for occurence and (Vph < 55%Vref || Vph > 75%Vref) for restoration
if(((g_TamperStatus&HALLSENS_MASK)==0)&&((g_TamperStatus&ACMAG_MASK)==0))
{
	if(((v[LINE_PHASE_R]<1795)&&(v[LINE_PHASE_R]>1325))||((v[LINE_PHASE_Y]<1795)&&(v[LINE_PHASE_Y]>1325))||((v[LINE_PHASE_B]<1795)&&(v[LINE_PHASE_B]>1325)))
	{
	...
	if(((v[LINE_PHASE_R]>1805)||(v[LINE_PHASE_R]<1315))&&((v[LINE_PHASE_Y]>1805)||(v[LINE_PHASE_Y]<1315))&&((v[LINE_PHASE_B]>1805)||(v[LINE_PHASE_B]<1315)))
	{
	...
	// Check overvoltage in any phase
// 115%Vref = 276V,  150%Vref = 360V
// MVVNL defines Over Voltage occurence as :
// (Vph > 115%Vref && Vph < 150%Vref) for occurence and (Vph < 115%Vref || Vph > 150%Vref) for restoration
if(((g_TamperStatus&HALLSENS_MASK)==0)&&((g_TamperStatus&ACMAG_MASK)==0))
{
	if(((v[LINE_PHASE_R]>2760)&&(v[LINE_PHASE_R]<3600))||((v[LINE_PHASE_Y]>2760)&&(v[LINE_PHASE_Y]<3600))||((v[LINE_PHASE_B]>2760)&&(v[LINE_PHASE_B]<3600)))
	{
	...
	// Check  voltage unbalance condition - all three phase voltages must be greater than 24 Volts 

// if all phases > 24Volts	and Vmxa - Vmin > 30%Vref (72V)
	if(phasesGT24==3)
	{
		maxValue = findMax(v[LINE_PHASE_R],v[LINE_PHASE_Y],v[LINE_PHASE_B]);
		minValue = findMin(v[LINE_PHASE_R],v[LINE_PHASE_Y],v[LINE_PHASE_B]);

		if((maxValue-minValue)>725)
		{
	    check_setTamper(&voltageUnbalanceCtr, computeMask(VOLTAGE_UNBALANCED),TAMPER_PERSISTENCE_ON,3,RECORD_DELAY);
		}

		if((maxValue-minValue)<715)
		{
...
// Iph>10%Ib and pf[ph]>0.5
	if(phasesGT24==3)
	{
		if((pf[LINE_PHASE_R] > 490)&&(i[LINE_PHASE_R]>95))
		{
			temp_power = (int32_t)(EM_GetActivePower(LINE_PHASE_R));
		  if (temp_power < 0){
		    check_setTamper(&ctReverseCtr[LINE_PHASE_R], computeMask(CT_REVERSED_R),TAMPER_PERSISTENCE_ON,6,LONG_RECORD_DELAY);
				currentreversectr++;}
		  else
		    check_clearTamper(&ctReverseCtr[LINE_PHASE_R], computeMask(CT_REVERSED_R),TAMPER_PERSISTENCE_OFF,6,LONG_RECORD_DELAY);
		}

		if((pf[LINE_PHASE_Y] > 490)&&(i[LINE_PHASE_Y]>95))
		{
			temp_power = (int32_t)(EM_GetActivePower(LINE_PHASE_Y));
		  if (temp_power < 0){
		    check_setTamper(&ctReverseCtr[LINE_PHASE_Y], computeMask(CT_REVERSED_Y),TAMPER_PERSISTENCE_ON,7,LONG_RECORD_DELAY);
				currentreversectr++;}
		  else
		    check_clearTamper(&ctReverseCtr[LINE_PHASE_Y], computeMask(CT_REVERSED_Y),TAMPER_PERSISTENCE_OFF,7,LONG_RECORD_DELAY);
		}
		
		if((pf[LINE_PHASE_B] > 490)&&(i[LINE_PHASE_B]>95))
		{
			temp_power = (int32_t)(EM_GetActivePower(LINE_PHASE_B));
		  if (temp_power < 0){
		    check_setTamper(&ctReverseCtr[LINE_PHASE_B], computeMask(CT_REVERSED_B),TAMPER_PERSISTENCE_ON,8,LONG_RECORD_DELAY);
				currentreversectr++;}
			else		
		    check_clearTamper(&ctReverseCtr[LINE_PHASE_B], computeMask(CT_REVERSED_B),TAMPER_PERSISTENCE_OFF,8,LONG_RECORD_DELAY);
		}
...
// also add if all three phase voltages > 24 volts
	if((mixedPowers==0)&&((g_TamperStatus&HALLSENS_MASK)==0)&&((g_TamperStatus&ACMAG_MASK)==0)&&(phasesGT24==3))
	{
		if(currentsGT145>=1)
		{
			maxValue = findMax(i[LINE_PHASE_R],i[LINE_PHASE_Y],i[LINE_PHASE_B]);
			
		  if(i[LINE_PHASE_R] < (maxValue/20))
			{
			  check_setTamper(&ctOpenCtr[LINE_PHASE_R], computeMask(CT_OPEN_R),TAMPER_PERSISTENCE_ON,11,RECORD_DELAY);
				ct_open_ctr++;
			}
		  else
		    check_clearTamper(&ctOpenCtr[LINE_PHASE_R], computeMask(CT_OPEN_R),TAMPER_PERSISTENCE_OFF,11,RECORD_DELAY);
				
		  if(i[LINE_PHASE_Y] < (maxValue/20))
			{
		    check_setTamper(&ctOpenCtr[LINE_PHASE_Y], computeMask(CT_OPEN_Y),TAMPER_PERSISTENCE_ON,12,RECORD_DELAY);
				ct_open_ctr++;
			}
		  else
		    check_clearTamper(&ctOpenCtr[LINE_PHASE_Y], computeMask(CT_OPEN_Y),TAMPER_PERSISTENCE_OFF,12,RECORD_DELAY);
				
		  if(i[LINE_PHASE_B] < (maxValue/20))
			{
				check_setTamper(&ctOpenCtr[LINE_PHASE_B], computeMask(CT_OPEN_B),TAMPER_PERSISTENCE_ON,13,RECORD_DELAY);
				ct_open_ctr++;
			}
		  else
...
// now check CT bypass		
		...
		{		
		  ...
		  if((ineutralDiff > 190)&&(i[LINE_PHASE_R]>18)&&(i[LINE_PHASE_Y]>18)&&(i[LINE_PHASE_B]>18)&&(1))
			{
	...
	// Check current unbalance condition			if((i[LINE_PHASE_R]>95)&&(i[LINE_PHASE_Y]>95)&&(i[LINE_PHASE_B]>95))
		...
		if((maxValue-minValue)>295)
		{// 30% of Ib
	    check_setTamper(&currentUnbalanceCtr, computeMask(CURRENT_UNBALANCED),TAMPER_PERSISTENCE_ON,5,RECORD_DELAY);
		}
		else
	    check_clearTamper(&currentUnbalanceCtr, computeMask(CURRENT_UNBALANCED),TAMPER_PERSISTENCE_OFF,5,RECORD_DELAY);
	
	  // Check over current condition - 72 amps 7200
		if((i[LINE_PHASE_R]>7190)||(i[LINE_PHASE_Y]>7190)||(i[LINE_PHASE_B]>7190))
		{
	    check_setTamper(&overCurrentCtr, computeMask(OVER_CURRENT),TAMPER_PERSISTENCE_ON,99,RECORD_DELAY);
//			fault_code=7;	// generic fault - each fault code bit has a meaning now - 7 means bit 0,1,2 are set which is Missing potential Vr, Vy, Vb
		}
		else
	    check_clearTamper(&overCurrentCtr, computeMask(OVER_CURRENT),TAMPER_PERSISTENCE_OFF,99,RECORD_DELAY);
	}
/*	
	else
	{
...
*/
		...
		acMagnetSecondsCtr++;
		if(acMagnetSecondsCtr==28)
		{// it is going to be thirty seconds soon
	    DI();
			acMagnetSecondsCtr=0;	// get ready for the next 30 seconds
			if(acMagnetPulseCtr<2)
			{
			...
			{// here 2 or more pulses have been received in 30 seconds
	...
	// if ac magnet is sensed then we should disable dc magnet detection - not doing this any more
//			magnetCtr=0;
