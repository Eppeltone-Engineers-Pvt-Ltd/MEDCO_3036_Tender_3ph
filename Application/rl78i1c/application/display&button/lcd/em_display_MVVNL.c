...
uint8_t displayHold;
uint8_t long_pressed_flag;
uint8_t backToAutoCount;
	...
	else
	{
		...
		else
		{
			if(g_display_mode==1)
			{
				...
				else if((long_pressed_flag==0)&&(backToAutoCount==0))
				{
					mode_timeout_ctr++;
					if(mode_timeout_ctr>299)// 299 
					{
						makeDisplayReadyForAutoScroll();
					}
				}
			...
			{// we are in high res mode
				mode_timeout_ctr++;
				if(mode_timeout_ctr>1799)	// 30 minutes
				{
					makeDisplayReadyForAutoScroll();
				}
			}
...
uint8_t fwdKeyLongPressedCtr;
uint8_t fwdKeyHandled;
uint8_t revKeyHandled;
uint8_t lockTimeoutCtr;
uint8_t revKeyLongPressedCtr;
uint8_t captured_state=0;
uint8_t unlockTimerCounter;
...
// 0 - autoscroll, 1 - displayOnDemand, 2 - High resolution display
	g_display_mode++;
	if(g_display_mode==3)	// this board now has High resolution display as requested by Shubham on 19/10/2023
	{
		g_display_mode=0;
		alt_autoscroll_mode=0;
		sub_disp_state=255;
...
void handleKeyLogic(void)
{
	...
	{// here both switches may not be pressed or either switch may be pressed
		...
		{
			bothKeysPressedCtr=0;
			revKeyLongPressedCtr=0;

			fwdKeyLongPressedCtr++;

			if ((fwdKeyHandled == 0) && (long_pressed_flag == 0))
			{
				someKeyPressed = 1;
				captured_state = disp_state;
				fwdKeyHandled = 1; // mark this press as handled
			}

			// Handle long press
			if (fwdKeyLongPressedCtr > 125)
			{
				if((g_display_mode!=0)&&(g_display_mode!=2))
				{
					...
					{
						LCD_ClearAll();
					LCD_DisplayDigit(2, LCD_CHAR_L);
					LCD_DisplayDigit(3, 0);
					LCD_DisplayDigit(4, LCD_CHAR_C);
					// First long press: Lock state
					
						long_pressed_flag = 1;
						someKeyPressed = 1;
						disp_state = captured_state;
						backToAutoCount=1;
					//	fwdKeyLongPressedCtr = 0;
					}
					else
					{
						// Second long press: Unlock state
						LCD_ClearAll();
						LCD_DisplayDigit(2, LCD_CHAR_U);
						LCD_DisplayDigit(3, LCD_CHAR_N);
						LCD_DisplayDigit(3, LCD_CHAR_L);
						LCD_DisplayDigit(4, 0);
						LCD_DisplayDigit(5, LCD_CHAR_C);
						long_pressed_flag = 0;
						someKeyPressed = 1;
					}
					fwdKeyLongPressedCtr = 0;
		...
		else if ((revKeyPressed == 1) && (fwdKeyPressed == 0))
		{
			bothKeysPressedCtr = 0;
			fwdKeyLongPressedCtr = 0;

			revKeyLongPressedCtr++;

			if ((revKeyHandled == 0) && (long_pressed_flag == 0))
			{
				someKeyPressed = 1;
				captured_state = disp_state;
				revKeyHandled = 1;
		...
		else
		{
			...
			// Reset long press counters
			fwdKeyLongPressedCtr = 0;
			revKeyLongPressedCtr = 0;
		}

		// === RESET HANDLED FLAGS ON KEY RELEASE ===
		if (fwdKeyPressed == 0)
			fwdKeyHandled = 0;
	...
	{
		if((fwdKeyPressed==0)&&(revKeyPressed==0))
		{
			unlockcnter++;
			if (unlockcnter>6995)
			{
		...
		{
			// Reset idle counter if any key is being pressed
			unlockcnter = 0;
		}
	}
}
		...
		{// here some key has been pressed
			...
			if(revKeyPressed==1)
			{
				...
				{// we need to go to the last state
					...
					{
						case 0:
							disp_state=23;	// last autoscroll state - 28 earlier , now 36
							break;
							
						case 1:
							disp_state=36;	// last pushbutton state - 34 earlier , now 36
							break;
    ...
    case 15:		// Existing tamper if any - last item
      ...
      break;
   	...
   	case 16:		// average pf for current month
			tempLong = computeAveragePF(myenergydata.kwh, myenergydata.kvah,0,0, IMPORT);
			displayPrepare();
			display_long_three_dp(tempLong);
...
//			tempLong = getMonthlyPowerOnMinutes(); // computes the difference between current and last month power on minutes 
			tempLong = ctldata.powerOnminutes_eb; 
...
//			tempLong = EM_GetPowerFactor(LINE_PHASE_B)*100;
    ...
    case 18:		// Current - R with direction
			tempLong = getInstantaneousParams(ICURRENTS,LINE_PHASE_R)*1000;
			display_long_three_dp(tempLong);
			LCD_DisplayDigit(9, LCD_CHAR_R);
			lcd_display_A();
    ...
    case 19:		// Current - Y with direction
			tempLong = getInstantaneousParams(ICURRENTS,LINE_PHASE_Y)*1000;
			display_long_three_dp(tempLong);
			LCD_DisplayDigit(9, LCD_CHAR_Y);
			lcd_display_A();
      ...
      break;
   	...
   	case 20:		// Current - B with direction
			tempLong = getInstantaneousParams(ICURRENTS,LINE_PHASE_B)*1000;
			display_long_three_dp(tempLong);
			LCD_DisplayDigit(9, LCD_CHAR_B);
			lcd_display_A();
	...
	case 29:	// Last occured Tamper date
	...
	case 30:	// Last occured Tamper time
			EPR_Read(LAST_OCCURED_DETAILS_ADDRESS,(uint8_t *)&datetime,sizeof(datetime));
			lcd_display_time(datetime.hour,datetime.minute,datetime.second);
			LCD_DisplayDigit(9, LCD_CHAR_O);
			LCD_DisplayDigit(8, LCD_CHAR_C);
	...
	case 32:	// Last restored Tamper date
	...
	case 33:	// Last restored Tamper time
			EPR_Read(LAST_RESTORED_DETAILS_ADDRESS,(uint8_t *)&datetime,sizeof(datetime));
			lcd_display_time(datetime.hour,datetime.minute,datetime.second);
			LCD_DisplayDigit(9, LCD_CHAR_R);
			LCD_DisplayDigit(8, 5);
    ...
    case 34:		// Total Tamper count
      ...
      break;
		...
		case 35:		// Phase Sequence - Voltage
			if((g_TamperStatus&MISSPOT_MASK)==0)
			{// here no potential is missing
				if(gPhaseSequence==0)
				{
     ...
     break;
		...
		case 36:		// Phase Sequence - Current
#ifdef CURRENT_PHASE_SEQUENCE		
		  i[LINE_PHASE_R] = (int16_t)(EM_EXT_GetIRMS(LINE_PHASE_R)*100);
		  i[LINE_PHASE_Y] = (int16_t)(EM_EXT_GetIRMS(LINE_PHASE_Y)*100);
		  i[LINE_PHASE_B] = (int16_t)(EM_EXT_GetIRMS(LINE_PHASE_B)*100);
...
// for CT-open use the MISS icon S_NM
	...
	if((((fault_code&_BV(11))!=0)||((fault_code&_BV(12))!=0)||((fault_code&_BV(13))!=0)||(g_TamperStatus&CTOPEN_MASK)!=0))
	{
...
// current_unbalance
	...
	else
	{
		...
		LCD_ClearSpSign(S_LEFT_ARROW);
	//	LCD_ClearSpSign(S_N);
		if((i[LINE_PHASE_R]>10)||(v[LINE_PHASE_R]>950))
			LCD_DisplaySpSign(S_R);	// current is also available
		if((i[LINE_PHASE_Y]>10)||(v[LINE_PHASE_Y]>950))
			LCD_DisplaySpSign(S_Y);	// current is also available
		if((i[LINE_PHASE_B]>10)||(v[LINE_PHASE_B]>950))
			LCD_DisplaySpSign(S_B);	// current is also available
		if(i[LINE_NEUTRAL]>10)
		LCD_DisplaySpSign(S_N);	// current is also available
...
//		switch(mytmprdata.latestTamperCode)	// this is obviously wrong
		{
			...
			case 15:	//	OVER_CURRENT
				lcd_msg((const uint8_t*)&msg_over_C);
				break;
